...
// git-watcher-plugin/src/index.ts
{
  ...
  autoApprovedCommands: [
    ...
    'find */workspaces/* *',
    'ls */workspaces/*',
    // Wrapper script (see Section 8) — 2 subdirectory levels: watcherId/downstreamId
...
interface Downstream {
  ...
  issueToken?: string          // optional: token for bot account issue creation
...
- Step 4 (per downstream): repo, branch, session strategy, agent, issueLabels, issueToken (optional), prompt template (default or open editor)
...
- Fields: `branch`, `strategy`, `agent`, `labels`, `issueToken`, `template`
...
- Default: omit webhook secrets and issueTokens
...
### Bot account for issue creation

If downstream has `issueToken` configured, plugin writes a wrapper script before spawning the session:

```bash
# dataDir/workspaces/<watcherId>/<downstreamId>/create_issue.sh
#!/bin/bash
GH_TOKEN="<issueToken>" gh issue create "$@"
```
...
System prompt instructs: "Create issues using `./create_issue.sh` instead of `gh issue create` directly."  
After session ends: `rm create_issue.sh`.

Token never appears in the LLM prompt/context.
