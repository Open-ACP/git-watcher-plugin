...
const plugin: OpenACPPlugin = {
  ...
  async setup(ctx: PluginContext): Promise<void> {
    ...
    //   category: 'plugin',
    //   async handler(args) {
    //     return { type: 'text', text: 'Hello from @openacp/git-watcher!' }
    //   },
    // })
  ...
  /**
   * Called when user runs `openacp plugin add @openacp/git-watcher`.
   * Use ctx.terminal for interactive prompts to gather configuration.
   */
  async install(ctx: InstallContext): Promise<void> {
    ctx.terminal.log.info('Installing @openacp/git-watcher...')
  ...
  /**
   * Called when user runs `openacp plugin configure @openacp/git-watcher`.
   * Re-run configuration prompts to update settings.
   */
  async configure(ctx: InstallContext): Promise<void> {
    ctx.terminal.log.info('Configuring @openacp/git-watcher...')
  ...
  /**
   * Called when user runs `openacp plugin remove @openacp/git-watcher`.
  ...
  async uninstall(ctx: InstallContext, opts: { purge: boolean }): Promise<void> {
    ctx.terminal.log.info('Uninstalling @openacp/git-watcher...')
    if (opts.purge) {
