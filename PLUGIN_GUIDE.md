...
**@openacp/git-watcher** is an OpenACP plugin.
...
async setup(ctx: PluginContext) {
  ...
  if (!apiKey) {
    ctx.log.warn('Not configured — run: openacp plugin configure @openacp/git-watcher')
    return
  }
  // Use apiKey...
}
...
describe('@openacp/git-watcher', () => {
  it('registers commands on setup', async () => {
    const ctx = createTestContext({ pluginName: '@openacp/git-watcher' })
  ...
  it('install saves settings', async () => {
    const ctx = createTestInstallContext({
      pluginName: '@openacp/git-watcher',
...
4. Users install with:
   ```bash
   openacp plugin install @openacp/git-watcher
   ```
